---  
title: "Two problems on Practical Statistics for Data Scientists"
...
```{r startup, warning=FALSE, message=FALSE, comment=NA}
library(tidyverse)
...
$H_0$: median annual income M = 75,000

$H_A$: median annual income M < 75,000
  
We will take oursignificance level for $\alpha$ as 0.05 ($5\%$). 
...
```
 ...
 In the below code, we calculate the p-value. Since $H_A$ is defined as the median to be the less that 75,000, the p-value corresponds to the left area of the observed median on the null distribution constructed above.
...
We fail to reject $H_0$ since the p-value (0.4279333) is above the significance level ($\alpha=0.05$), and conclude that the data do not provide convincing evidence that the median annual income of this university's faculty population is less than $75,000. 
...
Let's say the mean income for females is $\mu_1$ and males is $\mu_2$. Then our null and alternative hypotheses are:
...
Percentage of explained variability is 1.476260e+11/27.47e+11=5.37%.
F-statistic = 47.76128.
...
We are going to check if distribution of the response variable
is nearly normal within each group. In order to achieve that we will plot the histogram of each distribution. We are going to take the `log` of the income in order to make the histograms less skewed.
